...
cparams = {
    ...
    'image_size': 64, 'channels': 4, 'log_every_t': 200, 'first_stage_key': 'jpg',
    'monitor': 'val/loss_simple_ema', 'scale_factor': 0.18215, 'use_ema': False,

    'first_stage_config': {
        'embed_dim': 4, 'monitor': 'val/rec_loss', 
        'ddconfig': {
            'double_z': True, 'z_channels': 4,
            'resolution': 512, 'in_channels': 3,
            'out_ch': 3, 'ch_out': 128, 
...
def main():
    ...
    sd = pl_sd["state_dict"]
    ld_model = LatentDiffusion(**cparams)
    # for k, v in model.state_dict().items():
    #     print(f'{str(v.shape):30s}', k)
    m, u = ld_model.load_state_dict(sd, strict=False)
    ...
    # ld_model.model.diffusion_model.to(torch.float16).to(torch.device('cuda'))
    ...
    # start_code = None
    # if opt.fixed_code:
    start_code = torch.randn([opt.n_samples, opt.C, opt.H // opt.f, opt.W // opt.f], dtype=torch.float32, device=device)
            ...
            with ld_model.ema_scope():
                ...
                for n in trange(opt.n_iter, desc="Sampling"):
                    for prompts in tqdm(data, desc="data"):
                        uc = None
                        if opt.scale != 1.0:
                            uc = ld_model.get_learned_conditioning(batch_size * [""])
                        if isinstance(prompts, tuple):
                            prompts = list(prompts)
                        c = ld_model.get_learned_conditioning(prompts)
                        ...
                        samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                         ...
                                                         eta=opt.ddim_eta,
                                                         x_T=start_code)
                        # move the unet calc to cpu for reconstruction
                        # samples_ddim = samples_ddim.float().to('cpu')
